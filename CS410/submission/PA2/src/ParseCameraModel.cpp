...
ParseCameraModel::ParseCameraModel(char const *filename) {
	...
	// Parse focal length
	getline(inputfile, read_line);
	focal_length = strtof(read_line.substr(0, read_line.size()).c_str(), NULL);
...
Vertex * ParseCameraModel::ParseFocalPoint(string line_to_parse){
	...
	parsed_vertex->x_coordinate = strtof(line_to_parse.substr(last_pos, next_pos - last_pos).c_str(), &pEnd);
	...
	parsed_vertex->y_coordinate = strtof(line_to_parse.substr(last_pos, next_pos - last_pos).c_str(), &pEnd);

	last_pos = next_pos + 1;

	// next_pos = line_to_parse.find(" ", last_pos);

	parsed_vertex->z_coordinate = strtof(line_to_parse.substr(last_pos, line_to_parse.size()).c_str(), &pEnd);
...
void ParseCameraModel::VerifyCameraModel(void) {
	if ((fabsf(focal_length) < 0.00001) || (uMin > uMax) || (vMin > vMax)) {
