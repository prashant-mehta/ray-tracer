...
void WritePly::WriteFacesFromVector(vector<Face> inputfaces) {
	uint64_t number_of_faces = inputfaces.size();

	for (uint64_t i = 0; i < number_of_faces; i++){
		outputfile << inputfaces.at(i).getSizeOfFace() << " ";
		for (uint16_t j = 0; j < inputfaces.at(i).getSizeOfFace() ; j++) {
			outputfile << inputfaces.at(i).getVertexIdFromFace(j) << " ";
		}
		outputfile << "\n";
	}
