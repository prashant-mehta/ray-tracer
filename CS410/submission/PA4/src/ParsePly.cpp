...
ParsePly::ParsePly(string filename) {
	...
	ofstream outputfile;
	string attributes_filename = filename + "_attributes.txt";
	outputfile.open(attributes_filename.c_str());
	...
	properties_filename = filename + "_properties.txt";
	vertices_filename = filename + "_vertices.txt";
	vertex_properties_file.open(properties_filename.c_str());
	vertices_file.open(vertices_filename.c_str());
	...
	faces_filename = filename + "_faces.txt";
	outputfile.open(faces_filename.c_str());
...
void ParsePly::ParseAllVertices(string filename) {
	...
	vertices_input_file.open(filename.c_str());
...
void ParsePly::PrintVerticesMeanAndBounds(void){
	...
	this->ParseAllVertices(vertices_filename);
...
//	cout << "Number of properties are " << this->parseAttributesObj->GetNumberOfProperties() << endl;
	...
	vertex_to_add.z_coordinate = strtof(line_to_parse.substr(last_pos, line_to_parse.size()).c_str(), &pEnd);
 ...
 * */
void ParsePly::ParseFaces(string filename) {
	this->faces.clear();
	...
	faces_input_file.open(filename.c_str());
...
void ParsePly::ParseFaceVertices(string line_to_parse) {
	...
	if (face_to_add.getSizeOfFace() != size_of_face) {
		cout << "Error in parsing face vertices " << size_of_face << " " << face_to_add.getSizeOfFace() << endl;
		exit (EXIT_FAILURE);
	}
