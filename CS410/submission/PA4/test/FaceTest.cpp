...
namespace {
  ...
  TEST_F(FaceTest, VectorOfFaces) {
    	  ...
    	  vector<uint64_t> f0_indices = testVector.at(0).getFaceVertexIds();

    	  vector<uint64_t> f1_indices = testVector.at(1).getFaceVertexIds();

    	  vector<uint64_t> f2_indices = testVector.at(2).getFaceVertexIds();
  ...
  TEST_F(FaceTest, DefaultProperties) {
	  Vertex v1(12.0, -15.1234, 0.123, "0.145 8.945");
	  Vertex v2(12.0, -15.1234, 0.123, "0.145 8.945");
	  Vertex v3(12.0, -15.1234, 0.123, "0.145 8.945");
	  ...
	  float *dr = testObj->getDiffuseReflectance();

	  for (int i = 0; i < 3; i++)
		  ASSERT_FLOAT_EQ(dr[i],0.5);

	  ASSERT_FLOAT_EQ(testObj->getSpecularReflectance(),0);
	  ASSERT_FLOAT_EQ(testObj->getShininessExponent(),1);

  }

  TEST_F(FaceTest, SetProperties) {
	  Vertex v1(12.0, -15.1234, 0.123, "0.145 8.945");
	  Vertex v2(12.0, -15.1234, 0.123, "0.145 8.945");
	  Vertex v3(12.0, -15.1234, 0.123, "0.145 8.945");
	  ...
	  testObj->setDiffuseReflectance(0.23, 0.412, 0.0009);
	  testObj->setSpecularReflectance(0.0023);
	  testObj->setShininessExponent(15.23);

	  float *dr = testObj->getDiffuseReflectance();

	  ASSERT_FLOAT_EQ(dr[0],0.23);
	  ASSERT_FLOAT_EQ(dr[1],0.412);
	  ASSERT_FLOAT_EQ(dr[2],0.0009);

	  ASSERT_FLOAT_EQ(testObj->getSpecularReflectance(),0.0023);
	  ASSERT_FLOAT_EQ(testObj->getShininessExponent(),15.23);
