...
void ParsePly::ComputeRadius(void) {
	...
	for (int i = 0; i < number_of_vertices; i++) {
		...
		if (fabsf(distance) > max_distance) {
			max_distance = fabsf(distance);
		}
...
//	while((next_pos = line_to_parse.find(' ', last_pos)) != string::npos){
	...
	minimum_bounds->x_coordinate = fminf(vertex_to_add.x_coordinate, minimum_bounds->x_coordinate);
	minimum_bounds->y_coordinate = fminf(vertex_to_add.y_coordinate, minimum_bounds->y_coordinate);
	minimum_bounds->z_coordinate = fminf(vertex_to_add.z_coordinate, minimum_bounds->z_coordinate);

	maximum_bounds->x_coordinate = fmaxf(vertex_to_add.x_coordinate, maximum_bounds->x_coordinate);
	maximum_bounds->y_coordinate = fmaxf(vertex_to_add.y_coordinate, maximum_bounds->y_coordinate);
	maximum_bounds->z_coordinate = fmaxf(vertex_to_add.z_coordinate, maximum_bounds->z_coordinate);
