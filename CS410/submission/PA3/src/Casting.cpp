...
void Casting::CastingForModel(void) {
	...
	float d, dSquare, s, s1, s2, UdotT, UdotTSquare, Tsquare, NdotL, source_point_distance, VdotR;
	...
	for (int model_index = 0; model_index < this->number_of_models;
		...
		Vertex L, LminusE, OminusE, LightSource;
		Vertex * LightRay, *UnitLightRay;
		Vertex A, B, C;
		Vertex T;
		Vertex *SurfaceNormal;
		Vertex *RayPolygonToFP, * ReflectedRay;
...
*/
						if (t < tmin && t < pixel_colors.at((a - vMin) * uSize + (b - uMin)).distance_closest_intersection) {

							// t < all other intersections
							pixel_colors.at((a - vMin) * uSize + (b - uMin)).distance_closest_intersection = t;

							// Calculate pixel color from ambient light source
							AmbientReflection(a, b, all_triangles.at(k).getDiffuseReflectance(),
									this->parse_materials->light_sources.at(this->parse_materials->ambient_light_source_index));

							// Compute Point of intersection using t
							PointOfIntersection = ComputePointOfIntersection(L, t);
							...
							for (int curr_light = 0; curr_light	< this->parse_materials->light_sources.size(); curr_light++) {
								...
								if (this->parse_materials->light_sources.at(
									...
									if (ray_shadowed != true) {
										...
										DiffuseReflection(a, b, all_triangles.at(k).getDiffuseReflectance(), this->parse_materials->light_sources.at(curr_light), NdotL);

										SpecularReflection(a, b, all_triangles.at(k).getSpecularReflectance(), all_triangles.at(k).getShininessExponent(),
												this->parse_materials->light_sources.at(curr_light), VdotR);
...
Vertex * Casting::ComputePointOfIntersection(Vertex L, float t) {
Vertex * P = new Vertex();

P->x_coordinate = L.x_coordinate + (t * U->x_coordinate);
P->y_coordinate = L.y_coordinate + (t * U->y_coordinate);
P->z_coordinate = L.z_coordinate + (t * U->z_coordinate);
...
void Casting::AmbientReflection(uint32_t a, uint32_t b,
	float *diffuse_reflectance, LightSource ls) {
...
pixel_colors.at((a - vMin) * uSize + (b - uMin)).rValue = rV;
pixel_colors.at((a - vMin) * uSize + (b - uMin)).gValue = gV;
pixel_colors.at((a - vMin) * uSize + (b - uMin)).bValue = bV;

}

void Casting::DiffuseReflection(uint32_t a, uint32_t b, float *diffuse_reflectance, LightSource ls, float cosTheta) {
...
void Casting::SpecularReflection(uint32_t a, uint32_t b, float specular_reflectance, float shininess_exponent, LightSource ls, float VdotR) {
	float rV, gV, bV;
