...
//	  			  cout << result[i*2 + j] << endl;
  ...
  TEST_F(MatrixOperationsTest, TestGaussianElimination) {

	  testObj = new MatrixOperations();

	 float A[9] = {2, 1, 3, 2, 6, 8, 6, 8, 18};

	 float b[3] = {1, 3, 5};

	 float * result = testObj->GaussianElimination(A, b);


	 ASSERT_FLOAT_EQ(result[0], 0.3);
	 ASSERT_FLOAT_EQ(result[1], 0.4);
	 ASSERT_FLOAT_EQ(result[2], 0);

	 float C[9] = {1, -2, 3, 2, 1, 1, -3, 2, -2};

	 float d[3] = {7, 4, -10};

	 result = testObj->GaussianElimination(C, d);

	 ASSERT_FLOAT_EQ(result[0], 2);
	 ASSERT_FLOAT_EQ(result[1], -1);
	 ASSERT_FLOAT_EQ(result[2], 1);

  }

  TEST_F(MatrixOperationsTest, TestLUDecomposition) {
	 ...
	 float * result = testObj->LUDecomposition(A, b);
	 ...
	 result = testObj->LUDecomposition(C, d);
  ...
  TEST_F(MatrixOperationsTest, TestDetrminant) {
	 ...
	 float result = testObj->Determinant(A);


	 ASSERT_FLOAT_EQ(result, 40);

	 ASSERT_EQ(testObj->deteminant_zero, false);

	 float C[9] = {1, -1, 2, 2, 1, 1, 1, 1, 0};


	 result = testObj->Determinant(C);
	 cout << result << endl;

	 ASSERT_EQ(testObj->deteminant_zero, true);
