...
Vertex * VectorOperations::NonParallelUnitVector(Vertex *v){
	...
	if ( (nonParallel->x_coordinate < nonParallel->y_coordinate && nonParallel->x_coordinate < nonParallel->z_coordinate)
			|| (nonParallel->x_coordinate < 0.0000001 && nonParallel->y_coordinate < 0.0000001
					&& (nonParallel->z_coordinate - 1.0) < 0.0000001  )) {
		nonParallel->x_coordinate = 1.0;
	}
	else if ( (nonParallel->y_coordinate < nonParallel->z_coordinate &&
			nonParallel->y_coordinate < nonParallel->x_coordinate) || (nonParallel->y_coordinate < 0.0000001
					&& nonParallel->z_coordinate < 0.0000001 && (nonParallel->x_coordinate - 1.0) < 0.0000001  ) ) {
...
bool VectorOperations::testIfOrthogonal (Vertex *v1, Vertex *v2){
	...
	if (this->DotProduct(v1, v2) < 0.0000001) {
		return true;
	}
	else {
		return false;
...
bool VectorOperations::testIfUnitVector (Vertex *v){
	...
	if (fabs(this->Magnitude(v) - 1.00) < 0.0000001) {
		return true;
	}
	else {
		return false;
