...
TEST_F(VectorOperationsTest, TestScaledMagnitude) {
	testObj = new VectorOperations();
	Vertex *v1 = new Vertex(-0.153, 100.512, 2.0123, "test string");

	Vertex *v = testObj->Normalize(v1);

	v = testObj->ScaleVector(v, 2.5);

	ASSERT_FLOAT_EQ(testObj->Magnitude(v), 2.5);
...
TEST_F(VectorOperationsTest, TestVectorAddition) {
	testObj = new VectorOperations();
	Vertex *v1 = new Vertex(-7, -4, 3, "test string");
	...
	Vertex *v = testObj->AddVectors(v1, v2);

	ASSERT_FLOAT_EQ(v->x_coordinate, 10);
	ASSERT_FLOAT_EQ(v->y_coordinate, 2);
	ASSERT_FLOAT_EQ(v->z_coordinate, 5.5);
